...
import org.example.dto.CapacidadRequest;
...
public class PlasSbController {
    ...
    @Operation(
            summary = "Asigna una capacidad a una fecha dada",
            description = "En la fecha que se elija se guarda esta capacidad disponible",
            responses = {
    ...
    public HttpStatus post_capacidad_fecha(
            @Parameter(name = "fecha", description = "Fecha de la que se requiere la capacidad.", required = true, example = "01-01-2025")
            @RequestBody()CapacidadRequest dto
            ){
        try {
            SimpleDateFormat sdf = new SimpleDateFormat("dd-MM-yyyy");
            sdf.setLenient(false);
            Date fecha_format = sdf.parse(dto.getFecha());

            plasSbService.post_capacidad_fecha(dto.getCapacidad(), fecha_format);
